...
var emailYoungPatients = function () {
  ...
  if (appmodel.Form().Age() >= 20 && appmodel.Form().Age() <= 39) {
    message += emailText.optimalrisk.replace(
      "#optimalrisk#",
      "N/A* %0D%0A *This calculator only provides 10-year ASCVD and Optimal risk estimates for individuals 40-79 years of age."
    );
  } else {
    message += emailText.optimalrisk.replace(
      "#optimalrisk#",
  ...
  if (appmodel.recommendationType()) {
    message += emailText.advice
      .replace("#bpadvice#%0D%0A", "")
      .replace("#smokingadvice#%0D%0A", "")
      .replace("#aspirinadvice#", "")
      .replace("#diabetesadvice#%0D%0A", "")
