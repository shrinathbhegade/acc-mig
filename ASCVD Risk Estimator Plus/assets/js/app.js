...
var makeForecastSticky = function () {
  ...
  var diff1 = $(".sticky-holder")[0].clientHeight,
    diff2 =
      $(".sticky-holder")[0].clientHeight +
      $("#t1-score-banner")[0].clientHeight,
    diff3 =
      $(".sticky-holder")[0].clientHeight +
      $("#t1-score-banner")[0].clientHeight +
      $("#t2-score-banner")[0].clientHeight;

  var stickyClass = "stucked";
  diff2 += 0;
  diff3 += 0;
  if (
    $("#t1-score + div").css("display") !== "none" &&
    $(window).scrollTop() >= sticky1 - diff1
  ) {
  ...
  if (
    $("#t2-score + div").css("display") !== "none" &&
    $(window).scrollTop() >= sticky2 - diff2
  ) {
  ...
  if (
    $("#t3-score + div").css("display") !== "none" &&
    $(window).scrollTop() >= sticky3 - diff3
  ) {
...
var listchange = function () {
  ...
  $(".nav-list li").each(function () {
    var that = $(this);
    that[$("a", this).attr("href") === hash ? "addClass" : "removeClass"](
      "selected"
    );
  });
};
...
var getChartData = function () {
  ...
  if (appmodel.Form().VisitType()) {
    var initialRisk = parseFloat(
      appmodel.Form().TenYearRiskInitial().replace("%", "")
    );
    var followUpRisk = parseFloat(
      appmodel.Form().TenYearRisk().replace("%", "")
    );
    var optimalFollowup = parseFloat(
      appmodel.Form().RiskFormula.optimalFollowUpRisk().replace("%", "")
    );
    if ((initialRisk === followUpRisk) === optimalFollowup) {
      chartData.push(
        chartRiskSeries("Current/Initial/Optimal", initialRisk, tertiaryColor)
      );
    } else if (followUpRisk === optimalFollowup) {
      chartData.push(chartRiskSeries("Initial", initialRisk, primaryColor));
      chartData.push(
        chartRiskSeries("Current/ Optimal", followUpRisk, tertiaryColor)
      );
    } else if (initialRisk === optimalFollowup) {
      chartData.push(
        chartRiskSeries("Initial/ Optimal", initialRisk, tertiaryColor)
      );
      chartData.push(chartRiskSeries("Current", followUpRisk, secondaryColor));
    } else if (initialRisk === followUpRisk) {
      chartData.push(
        chartRiskSeries("Initial/ Current", initialRisk, primaryColor)
      );
      chartData.push(
        chartRiskSeries("Optimal", optimalFollowup, tertiaryColor)
      );
    } else {
      chartData.push(chartRiskSeries("Initial", initialRisk, primaryColor));
      chartData.push(chartRiskSeries("Current", followUpRisk, secondaryColor));
    ...
    var optimalRisk = parseFloat(
      appmodel.Form().RiskFormula.optimalBaselineRisk().replace("%", "")
    );
    var forecastData = getForecastChartData(initialRisk);
    if (initialRisk === optimalRisk) {
      chartData.push(
        chartRiskSeries("Current/ Optimal", initialRisk, tertiaryColor)
      );
    } else {
  ...
  chartData.forEach(function (chartItem) {
    if (chartItem.isForecast) {
      ...
      chartItem.name =
        chartItem.name.substring(0, chartItem.name.length - 5) + "*";
  ...
  realChartData.splice(
    0,
    0,
    chartRiskSeries(
      "DONOTSHOW",
      parseFloat(realChartData[0].data[0]) / 3,
      "white"
    )
  );
  ...
  for (var i = realChartData.length - 1; i >= 0; i--) {
    ...
    if (chartItem.name != "DONOTSHOW") {
      ...
      scatterArray.push(
        chartScatterRiskSeries(
          chartItem.name,
          chartItem.treatmentDescription,
          lastScatter,
          chartItem.isForecast,
          chartItem.data[0]
        )
      );
    }
      ...
      if (scatterArray[i].originalData === data) {
        scatterArray.push(
          chartScatterRiskSeries(
            chartItem.name,
            chartItem.treatmentDescription,
            scatterArray[i].data[0][1],
            true,
            scatterArray[i].originalData
          )
        );
        break;
      }
    ...
    Highcharts.chart("container", {
      ...
      tooltip: {
        ...
        formatter: function () {
          if (this.x === -0.26) {
            return (
              "<div class='chart-tooltip'><span><b>Forecasted Risk:</b><span> " +
              this.series.userOptions.originalData +
              "% if patient <br><span style='word-wrap: break-word;'>" +
              this.series.userOptions.treatmentDescription +
              "</span></div>"
            );
          } else if (this.x === 0.26) {
            return (
              "<div class='chart-tooltip'><span><b>" +
              this.series.name +
              " Risk:</b> " +
              this.series.userOptions.originalData +
              "%</span><div>"
...
var therapyCheckboxLock = function () {
  ...
  if (
    (appmodel.Form().BloodPressure() <= 119 && !appmodel.Form().isDiabetic()) ||
    (appmodel.Form().BloodPressure() <= 129 && appmodel.Form().isDiabetic())
  ) {
    ...
    var isBPMedication = ko.utils.arrayFirst(
    ...
    if (isBPMedication) {
      appmodel
        .Form()
        .TratmentOne_selected.remove(appmodel.FormData.treatmentOne[2]);
    }
    isBPMedication = ko.utils.arrayFirst(
    ...
    if (isBPMedication) {
      appmodel
        .Form()
        .TratmentTwo_selected.remove(appmodel.FormData.treatmentTwo[2]);
    }
    isBPMedication = ko.utils.arrayFirst(
    ...
    if (isBPMedication) {
      appmodel
        .Form()
        .TratmentThree_selected.remove(appmodel.FormData.treatmentThree[2]);
    }
  } else {
  ...
  if (appmodel.Form().RiskFormula.ForecastRiskAspirin() == "NA") {
    ...
    var isBPMedication = ko.utils.arrayFirst(
      appmodel.Form().TratmentOne_selected(),
      function (item) {
    ...
    if (isBPMedication) {
      appmodel
        .Form()
        .TratmentOne_selected.remove(appmodel.FormData.treatmentOne[3]);
    }
    isBPMedication = ko.utils.arrayFirst(
      appmodel.Form().TratmentTwo_selected(),
      function (item) {
        return item.therapyText === "Aspirin Therapy";
      }
    );
    if (isBPMedication) {
      appmodel
        .Form()
        .TratmentTwo_selected.remove(appmodel.FormData.treatmentTwo[3]);
    }
    isBPMedication = ko.utils.arrayFirst(
      appmodel.Form().TratmentThree_selected(),
      function (item) {
        return item.therapyText === "Aspirin Therapy";
      }
    );
    if (isBPMedication) {
      appmodel
        .Form()
        .TratmentThree_selected.remove(appmodel.FormData.treatmentThree[3]);
    }
  } else {
  ...
  if (appmodel.Form().RiskFormula.ForecastRiskStatin() == "NA") {
    ...
    if (isBPMedication) {
      appmodel
        .Form()
        .TratmentOne_selected.remove(appmodel.FormData.treatmentOne[1]);
    }
    isBPMedication = ko.utils.arrayFirst(
      appmodel.Form().TratmentTwo_selected(),
      function (item) {
    ...
    if (isBPMedication) {
      appmodel
        .Form()
        .TratmentTwo_selected.remove(appmodel.FormData.treatmentTwo[1]);
    }
    isBPMedication = ko.utils.arrayFirst(
      appmodel.Form().TratmentThree_selected(),
      function (item) {
    ...
    if (isBPMedication) {
      appmodel
        .Form()
        .TratmentThree_selected.remove(appmodel.FormData.treatmentThree[1]);
    }
  } else {
...
var tabchange = function (data) {
  ...
  $(".tooltip").hide();
  if (
    !appmodel.recommendationUnlock() &&
    (hash.indexOf("advice") || hash.indexOf("therapy"))
  ) {
    window.location.href = "#!/calculate/estimate/";
  }
  if (
    hash.indexOf("estimate") > -1 ||
    hash.indexOf("therapy") > -1 ||
    hash.indexOf("advice") > -1
  ) {
...
var errorTextBoxScroll = function () {
  ...
  if (warningBoxTop > errorBoxTop) {
    ...
    if (errorBox.length != 0 && warningBox.length != 0) {
      if (
        $(errorBox.get(0)).offset().left < $(warningBox.get(0)).offset().left
      ) {
        $(errorBox.get(0)).find("input").focus();
      } else {
...
var nestedPanelVisibleToggle = function (data) {
  ...
  if ($(".nested-accordion " + data).hasClass("nested-selected")) {
    ...
    $(".nested-accordion " + data + " .nested-collapsable-panel").removeClass(
      "nested-selected"
    );
  } else {
    $(".nested-accordion " + data).addClass("nested-selected");
    $(".nested-accordion " + data + " .nested-collapsable-panel").addClass(
      "nested-selected"
    );
  }
  ...
  $(target).slideToggle();
  if (
    $("#" + parentSection + " .nested-collapsable-panel.selected").length != 0
  ) {
    $("#" + parentSection + "_action").attr("data-action", "collapse");
    $("#" + parentSection + "_action").html("");
    $("#" + parentSection + "_action").html("Collapse All");
...
function formattedHTML(isForecast, value, text) {
  var header =
    "<div style='margin-top: -15px; margin-left: #OFFSET#5px; word-wrap: break-word; margin-right: 5px;font-family: Open Sans, Helvetica Neue, Helvetica, Helvetica, Arial, sans-serif;'>";
  var footer = "";
  if (isForecast) {
  ...
  return (
    header.replace("#OFFSET#", viewWidth() <= 400 && isForecast ? "-1" : "") +
    footer
  );
}
...
function chartScatterRiskSeries(
  name,
  treatmentDesc,
  data,
  isForecast,
...
function getForecastChartData(currentRisk) {
  ...
  if (
    ...
    chartData.push(
      chartForecastSeries(
        unifyTreatmentScenario(treatmentCaption1, treatmentCaption2),
        treatment1
      )
    );
    ...
    chartData.push(
      chartForecastSeries(
        unifyTreatmentScenario(treatmentCaption2, treatmentCaption3),
        treatment2
      )
    );
    ...
    chartData.push(
      chartForecastSeries(
        unifyTreatmentScenario(treatmentCaption1, treatmentCaption3),
        treatment1
      )
    );
...
var panzoom = function (currentPanzoomClass) {
  ...
  var $scaleZoom =
    $(".parent.panzoom").width() / $(".panzoom-element img").width();
...
ko.bindingHandlers.addCSS = {
  init: function (
    ...
    for (var prop in obj) {
      if (obj[prop] == true) $(element).addClass(prop);
      else $(element).removeClass(prop);
    }
  ...
  $("[custom-tooltip-parent]").each(function (index, element) {
    ...
    var assignTop = function () {
      ...
      if (tooltip.hasClass("bottom")) {
        tooltip.css(
          "top",
          jqElement.position().top + jqElement.parent().height()
        );
      }
...
function printApp() {
  ...
  //Graph Card
  printerStage.appendChild(
    document.getElementById("graph-card").cloneNode(true)
  );
  ...
  $(clone)
    .find("span.sub-line")
    .html(
      "The advice below represents suggestions for clinicians on lowering a patient’s ASCVD risk, based on ACC/AHA clinical policy (see Resources/References in the app)."
    );
  $(clone).addClass("row columns");
  printerStage.appendChild(clone);
...
function AddEvent(d) {
  ...
  d.addEventListener("keydown", function (event) {
    if (event.which === 9 || event.which === 13) isTabPressed = true;
  ...
  function preventAlphabets(e) {
    if (e.type == "paste") {
      ...
      if (
        e.clipboardData.getData != undefined &&
    ...
    if (e.type == "keypress") {
      ...
      if (e.shiftKey) {
        ...
        //0=>48, 9=>57, .=>46, ,=>44, Backspace=>8, Enter=>13, Tab=>0
        if (
          (k >= 48 && k <= 57) ||
          k == 46 ||
          k == 44 ||
