...
function formObject(parent) {
  ...
  self.UnitOfMeasure = ko.observable(false).extend({ rateLimit: 10 }); //rateLimit added because of compare value resetting due to synchronous knockout value update
  ...
  self.TotalCholesterolValue.subscribe(function (newValue) {
    if (newValue != undefined)
      self.TotalCholesterolValue(
        self.TotalCholesterolValue().replace(/,/g, ".")
      );
  });
  ...
  self.HDLCholesterol = ko.pureComputed(function () {
    ...
    var m = self.UnitOfMeasure() ? 0.0259 : 1;
    var value = i / m;
    return value;
  }, self);
  ...
  self.BaselineTotalCholesterolValue.subscribe(function (newValue) {
    if (newValue != undefined)
      self.BaselineTotalCholesterolValue(
        self.BaselineTotalCholesterolValue().replace(/,/g, ".")
      );
  });
  self.BaselineLDLCholesterolValue.subscribe(function (newValue) {
    if (newValue != undefined)
      self.BaselineLDLCholesterolValue(
        self.BaselineLDLCholesterolValue().replace(/,/g, ".")
      );
  });
  self.BaselineHDLCholesterolValue.subscribe(function (newValue) {
    if (newValue != undefined)
      self.BaselineHDLCholesterolValue(
        self.BaselineHDLCholesterolValue().replace(/,/g, ".")
      );
  });
  ...
  self.VisitTypeUI.subscribe(function () {
    if (self.VisitTypeUI() == undefined) return;
  ...
  self.riskRange = ko.pureComputed(function () {
    var Risk =
      self.TenYearCurrentRisk() != undefined
        ? parseFloat(self.TenYearCurrentRisk())
        : undefined;
    var InvalidLDLRange =
      self.LDLCholesterol() <= 69 || self.LDLCholesterol() >= 190;
  ...
  self.previousRiskRange = ko.pureComputed(function () {
    var previousRisk = parseFloat(
      self.TenYearRiskInitial().slice(0, self.TenYearRiskInitial().length - 1)
    );
  ...
  self.computedValuesAvailable = ko.pureComputed(function () {
    var formatRegex = new RegExp("^([0-9]{1,3}|0)([,.]{1}[0-9]{1,3})?$"),
      ...
      quitSmokingSelected =
        self.Smoker() == "Former" && self.QuiteSmokingMonths() == undefined
          ? false
          : true;

    var statinAspirinAvailable = self.VisitType()
      ? (self.OnAspirin() != undefined || self.OnAspirin() != null) &&
        (self.OnStatin() != undefined || self.OnStatin() != null)
      : true;
    if (
      ...
      self.HDLCholesterol() <=
        100 /*&& ldlCholesterol >= 30 && ldlCholesterol <= 300 && (parseFloat(ldlCholesterol.toFixed(3)) <= parseFloat((self.TotalCholesterol() - self.HDLCholesterol()).toFixed(3)))*/ &&
      self.BloodPressure() != null &&
      self.BloodPressure() >= 90 &&
      self.BloodPressure() <=
        200 /*&& self.DBloodPressure() != null && self.DBloodPressure() >= 60 && self.DBloodPressure() <= 130*/ &&
      self.TotalCholesterol() != null &&
      self.TotalCholesterol() >= 130 &&
      self.TotalCholesterol() <= 320 &&
      formatRegex.test(self.TotalCholesterolValue()) &&
      /*formatRegex.test(self.LDLCholesterolValue()) &&*/ formatRegex.test(
        self.HDLCholesterolValue()
      ) &&
      bpFormatReg.test(
        self.BloodPressure()
      ) /*&& bpFormatReg.test(self.DBloodPressure())*/ &&
  ...
  self.computedBaselineValuesAvailable = ko.pureComputed(function () {
    ...
    if (
      ...
      self.BaselineAge() >= 40 &&
      self.BaselineAge() <= 79 &&
      self.BaselineAge() <= self.Age() &&
      self.BaselineBloodPressure() != null &&
      self.BaselineBloodPressure() >= 90 &&
      self.BaselineBloodPressure() <= 200 &&
      self.BaselineHDLCholesterol() >= 20 &&
      self.BaselineHDLCholesterol() <= 100 &&
      self.BaselineLDLCholesterol() >= 30 &&
      self.BaselineLDLCholesterol() <= 300 &&
      self.BaselineTotalCholesterol() >= 130 &&
      parseFloat(self.BaselineLDLCholesterol().toFixed(3)) <=
        parseFloat(
          (
            self.BaselineTotalCholesterol() - self.BaselineHDLCholesterol()
          ).toFixed(3)
        ) &&
      ldlCholesterol >= 30 &&
      ldlCholesterol <= 300 &&
      parseFloat(ldlCholesterol.toFixed(3)) <=
        parseFloat(
          (self.TotalCholesterol() - self.HDLCholesterol()).toFixed(3)
        ) &&
      self.BaselineTotalCholesterol() <= 320 &&
      formatRegex.test(self.BaselineTotalCholesterolValue()) &&
      formatRegex.test(self.BaselineLDLCholesterolValue()) &&
      formatRegex.test(self.BaselineHDLCholesterolValue()) &&
      formatRegex.test(self.LDLCholesterolValue()) &&
      bpFormatReg.test(self.BaselineBloodPressure()) &&
      (self.BaselineHypertension() != null ||
        self.BaselineHypertension() != undefined) &&
      self.Race() &&
      self.Sex() &&
      ((self.Diabetic() == "Yes" &&
        (self.BaselineDiabetic() != null ||
          self.BaselineDiabetic() != undefined)) ||
        self.Diabetic() == "No") &&
      self.BaselineSmoker() != undefined &&
      self.BaselineSmoker() != null
    ) {
      return true;
    } else {
      return false;
  ...
  self.isDiabetic = ko.pureComputed(function () {
    var i = false;
    if (self.VisitType()) {
      if (self.Diabetic() == "Yes" || self.BaselineDiabetic() == "Yes")
        i = true;
    } else {
  ...
  self.isHypertension = ko.pureComputed(function () {
    var i = false;
    if (self.Hypertension() == "Yes" || self.BaselineHypertension() == "Yes")
      i = true;
    return i;
  }, self);
  ...
  self.TenYearRisk = ko.pureComputed(function () {
    ...
    if (
      self.computedValuesAvailable() &&
      !self.VisitType() &&
      !isNaN(self.RiskFormula.cvdPredict(true))
    ) {
      ...
      self.computedValuesAvailable() &&
      self.computedBaselineValuesAvailable() &&
      self.VisitType()
    ) {
  ...
  self.TreatmentOneScore = ko.pureComputed(function () {
    ...
    if (self.TratmentOne_selected().length > 0) {
      Score = self.CalculateForecastRisk(
  ...
  self.CalculateForecastRisk = function (value, isFollowup) {
    ...
    if (isFollowup) {
      ...
      score = self.RiskFormula.TenYearForecastedFollowUpRisk(
        isSmoking,
        isBP,
        isAspirin
      );
  ...
  self.computedValuesAvailableForLifetimeRisk = ko.pureComputed(function () {
    ...
    if (
      ...
      self.DBloodPressure() != null &&
      self.DBloodPressure() >= 60 &&
      self.DBloodPressure() <= 130 &&
      self.TotalCholesterol() != null &&
      self.TotalCholesterol() >= 130 &&
      self.TotalCholesterol() <= 320 &&
      formatRegex.test(self.TotalCholesterolValue()) &&
  ...
  self.invalidLdlMessage = ko.pureComputed(function () {
    //resizeHeight();
    if (
      self.LDLCholesterol() >= 190 &&
      !isNaN(parseFloat(self.LDLCholesterolValue()))
    ) {
  ...
  self.optimalRisk = ko.pureComputed(function () {
    return self.VisitType()
      ? self.RiskFormula.optimalFollowUpRisk()
      : self.RiskFormula.optimalBaselineRisk();
  }, self);
  self.validOptimalRiskValue = ko.pureComputed(function () {
    var i = "~";
    if (self.LDLCholesterol() >= 190) {
      return i;
    } else return self.optimalRisk();
  }, self);
  self.major = ko.computed(function () {
    var tc = self.TotalCholesterol();
    var bp = self.BloodPressure();
    var dbp = self.DBloodPressure();
    if (self.TotalCholesterol() == "") tc = undefined;
    if (self.BloodPressure() == "") bp = undefined;
    if (self.DBloodPressure() == "") dbp = undefined;

    var i =
      (tc >= 240 ? 1 : 0) +
      (bp >= 160 ? 1 : 0) +
      (dbp >= 100 ? 1 : 0) +
      (self.isHypertension() ? 1 : 0) +
      (self.isSmoker() ? 1 : 0) +
      (self.isDiabetic() ? 1 : 0);
    return i;
  }, self);
  self.elevated = ko.computed(function () {
    ...
    var i =
      ((tc >= 200 && tc < 240 ? 1 : 0) +
        (dbp >= 90 && dbp < 100 ? 1 : 0) +
        (bp >= 140 && bp < 160 && self.isHypertension() == false ? 1 : 0) >=
      1
        ? 1
        : 0) * (self.major() == 0 ? 1 : 0);
    return i;
  ...
  self.allOptimal = ko.computed(function () {
    ...
    i =
      ((tc < 180 ? 1 : 0) +
        (bp < 120 ? 1 : 0) +
        (dbp < 80 ? 1 : 0) * (self.isHypertension() ? 0 : 1) ==
      3
        ? 1
        : 0) * (self.major() == 0 ? 1 : 0);
    return i;
  ...
  self.notOptimal = ko.computed(function () {
    ...
    var i =
      ((tc >= 180 && tc < 200 ? 1 : 0) +
        (dbp >= 80 && dbp < 100 ? 1 : 0) +
        (bp >= 120 && bp < 140 && self.isHypertension() == false ? 1 : 0)) *
        (self.elevated() == 0 ? 1 : 0) *
        (self.major() == 0 ? 1 : 0) >=
      1
        ? 1
  ...
  self.lookupASCVD = function (category) {
    if (self.Sex() != null)
      return eval(
        "self.ascvdTable." + self.Sex().toLowerCase() + "." + category + ""
      );
    return "~";
  };
  self.ascvdTable = {
...
function viewModel() {
  ...
  self.glossaryQuery.subscribe(function (value) {
    ...
    for (var i in self.Glossary) {
      if (
        self.Glossary[i].label.toLowerCase().indexOf(value.toLowerCase()) >= 0
      ) {
        self.glossaryItems.push(self.Glossary[i]);
      }
    }
  ...
  self.hypertensionValidation = ko.pureComputed(function () {
    if (self.Form().Hypertension() == null && self.validate() == true) {
      ...
      self.Form().Hypertension() == "No" &&
      self.Form().BaselineHypertension() == "Yes" &&
      self.isFollowUp()
    ) {
  ...
  self.ageValidation = ko.pureComputed(function () {
    ...
    if (self.Form().Age() == null && self.validate() == true) {
      ...
      self.Form().Age() != null &&
      !new RegExp("^[0-9]{1,2}[.]{0,1}[0-9]{1,2}$").test(self.Form().Age())
    ) {
      i = self.FormData.notifications.age[1];
    } else if (
      self.Form().Age() != null &&
      (self.Form().Age() < 20 || self.Form().Age() > 79)
    ) {
      i = self.Form().UnitOfMeasure()
        ? self.FormData.notifications.age[1]
        : self.FormData.notifications.age[1];
    } else if (
      self.Form().Age() != null &&
      self.Form().Age() >= 20 &&
      ...
      self.Form().Age() &&
      (self.Form().Age() > 59 || self.Form().Age() < 40) &&
  ...
  self.invalidAgeValue = ko.pureComputed(function () {
    //resizeHeight();
    return self.ageValidation().id === 2 && !isNaN(self.Form().Age())
      ? true
      : false;
  }, self);
  ...
  self.hdlValidation = ko.pureComputed(function () {
    ...
    if (self.Form().HDLCholesterolValue() == null && self.validate() == true) {
      ...
      self.Form().HDLCholesterolValue() != null &&
      !new RegExp("^([0-9]{1,3}|0)([,.]{1}[0-9]{1,3})?$").test(
        self.Form().HDLCholesterolValue()
      )
    ) {
      i = self.FormData.notifications.hdlCholesterol[3];
    } else if (
      self.Form().HDLCholesterolValue() != null &&
      (self.Form().HDLCholesterol() < 20 || self.Form().HDLCholesterol() > 100)
    ) {
      i = self.Form().UnitOfMeasure()
        ? self.FormData.notifications.hdlCholesterol[2]
        : self.FormData.notifications.hdlCholesterol[1];
    } else {
      i = self.FormData.notifications.blank[0];
    }
    return i;
  ...
  self.totalCholesterolValidation = ko.pureComputed(function () {
    var i = self.FormData.notifications.blank[0];
    if (
      self.Form().TotalCholesterolValue() == null &&
      self.validate() == true
    ) {
      ...
      self.Form().TotalCholesterolValue() != null &&
      !new RegExp("^([0-9]{1,3}|0)([,.]{1}[0-9]{1,3})?$").test(
        self.Form().TotalCholesterolValue()
      )
    ) {
      i = self.FormData.notifications.totalCholesterol[3];
    } else if (
      self.Form().TotalCholesterolValue() != null &&
      (self.Form().TotalCholesterol() < 130 ||
        self.Form().TotalCholesterol() > 320)
    ) {
      //self.Form().TotalCholesterolValue(Math.round(self.Form().TotalCholesterolValue()));
      i = self.Form().UnitOfMeasure()
        ? self.FormData.notifications.totalCholesterol[2]
        : self.FormData.notifications.totalCholesterol[1];
    } else {
      i = self.FormData.notifications.blank[0];
    }
    return i;
  ...
  self.ldlValidation = ko.pureComputed(function () {
    ...
    if (self.Form().LDLCholesterolValue() == null && self.validate() == true) {
      ...
      self.Form().LDLCholesterolValue() != null &&
      !formatRegex.test(self.Form().LDLCholesterolValue())
    ) {
      i = self.FormData.notifications.ldlCholesterol[4];
    } else if (
      self.Form().LDLCholesterolValue() != null &&
      (self.Form().LDLCholesterol() < 30 || self.Form().LDLCholesterol() > 300)
    ) {
      i = self.Form().UnitOfMeasure()
        ? self.FormData.notifications.ldlCholesterol[2]
        : self.FormData.notifications.ldlCholesterol[1];
    } else if (
      self.Form().LDLCholesterolValue() != null &&
      parseFloat(self.Form().LDLCholesterol().toFixed(3)) >
        parseFloat(
          (
            self.Form().TotalCholesterol() - self.Form().HDLCholesterol()
          ).toFixed(3)
        )
    ) {
      i = self.Form().UnitOfMeasure()
        ? self.FormData.notifications.ldlCholesterol[3]
        : self.FormData.notifications.ldlCholesterol[3];
    } else {
      i = self.FormData.notifications.blank[0];
    }
  ...
  self.ldlValidation.subscribe(function () {
    if (
      self.Form().LDLCholesterol() >= 190 ||
      self.Form().LDLCholesterol() <= 69
    ) {
      self.resetBaselineValues();
      self.Form().VisitTypeUI(undefined);
      self.Form().VisitType(false);
  ...
  self.bloodPresureValidation = ko.pureComputed(function () {
    if (self.Form().BloodPressure() == null && self.validate() == true) {
      ...
      self.Form().BloodPressure() != null &&
      !new RegExp("^[0-9]{1,3}$").test(self.Form().BloodPressure())
    ) {
      return self.FormData.notifications.bloodPresure[2];
    } else if (
      self.Form().BloodPressure() != null &&
      (self.Form().BloodPressure() < 90 || self.Form().BloodPressure() > 200)
    ) {
  ...
  self.DBloodPressureValidation = ko.pureComputed(function () {
    if (self.Form().DBloodPressure() == null && self.validate() == true) {
      ...
      self.Form().DBloodPressure() != null &&
      !new RegExp("^[0-9]{1,3}$").test(self.Form().DBloodPressure())
    ) {
      return self.FormData.notifications.dbloodPresure[2];
    } else if (
      self.Form().DBloodPressure() != null &&
      (self.Form().DBloodPressure() < 60 || self.Form().DBloodPressure() > 130)
    ) {
  ...
  self.recommendationUnlock = ko.pureComputed(function () {
    var dbpAvailable,
      ...
      statinAvailable,
      aspirinAvailable,
      onStatin = self.Form().VisitType()
        ? self.Form().OnStatin()
        : "not required",
      onAspirin = self.Form().VisitType()
        ? self.Form().OnAspirin()
        : "not required",
    ...
    dbpAvailable =
      !isNaN(self.Form().DBloodPressure()) &&
      bpformatRegex.test(self.Form().DBloodPressure()) &&
      self.Form().DBloodPressure() >= 60 &&
      self.Form().DBloodPressure() <= 130
        ? true
        : false;
    ldlAvailable =
      !isNaN(ldlCholesterol) &&
      formatRegex.test(self.Form().LDLCholesterolValue()) &&
      ldlCholesterol >= 30 &&
      ldlCholesterol <= 300 &&
      parseFloat(ldlCholesterol.toFixed(3)) <=
    ...
    if (
      self.Form().computedValuesAvailable() &&
      self.isFollowUp() &&
      self.Form().computedBaselineValuesAvailable() &&
      self.Form().Age() >= 40 &&
      self.Form().Age() <= 79
    ) {
  ...
  self.NavigateToForecastPage = function (data) {
    if (self.Form().VisitTypeUI() == false) {
      ...
      self.recommendationUnlock() &&
      self.Form().VisitType() == false &&
      self.Form().invalidLdlMessage()
    ) {
      self.validate(false);
      self.baselineValidate(false);
      window.location.href = "#!/calculate/therapy/";
  ...
  self.NavigateToSummaryPage = function (data) {
    if (self.recommendationUnlock() && self.Form().VisitTypeUI() != undefined) {
      self.validate(true);
      self.baselineValidate(true);
  ...
  self.baselineAgeValidation = ko.computed(function () {
    ...
    if (self.Form().BaselineAge() == null && self.baselineValidate() == true) {
      ...
      self.Form().BaselineAge() != null &&
      !new RegExp("^[0-9]{1,2}[.]{0,1}[0-9]{1,2}$").test(
        self.Form().BaselineAge()
      )
    ) {
      i = self.FormData.notifications.baselineAge[1];
    } else if (
      self.Form().BaselineAge() != null &&
      (self.Form().BaselineAge() < 40 || self.Form().BaselineAge() > 79)
    ) {
      i = self.Form().UnitOfMeasure()
        ? self.FormData.notifications.age[1]
        : self.FormData.notifications.baselineAge[1];
    } else if (
      self.Form().BaselineAge() != null &&
      self.Form().Age() &&
      self.Form().BaselineAge() > self.Form().Age()
    ) {
  ...
  self.baselineTotalCholesterolValidation = ko.pureComputed(function () {
    var i = self.FormData.notifications.blank[0];
    if (
      self.Form().BaselineTotalCholesterolValue() == null &&
      self.baselineValidate() == true
    ) {
      ...
      self.Form().BaselineTotalCholesterolValue() != null &&
      !new RegExp("^([0-9]{1,3}|0)([,.]{1}[0-9]{1,3})?$").test(
        self.Form().BaselineTotalCholesterolValue()
      )
    ) {
      i = self.FormData.notifications.baselineTotalCholesterol[3];
    } else if (
      self.Form().BaselineTotalCholesterolValue() != null &&
      (self.Form().BaselineTotalCholesterol() < 130 ||
        self.Form().BaselineTotalCholesterol() > 320)
    ) {
      i = self.Form().UnitOfMeasure()
        ? self.FormData.notifications.baselineTotalCholesterol[2]
        : self.FormData.notifications.baselineTotalCholesterol[1];
    } else {
      i = self.FormData.notifications.blank[0];
    }
    return i;
  ...
  self.baselineHdlValidation = ko.pureComputed(function () {
    var i = self.FormData.notifications.blank[0];
    if (
      self.Form().BaselineHDLCholesterolValue() == null &&
      self.baselineValidate() == true
    ) {
      ...
      self.Form().BaselineHDLCholesterolValue() != null &&
      !new RegExp("^([0-9]{1,3}|0)([,.]{1}[0-9]{1,3})?$").test(
        self.Form().BaselineHDLCholesterolValue()
      )
    ) {
      i = self.FormData.notifications.baselineHdlCholesterol[3];
    } else if (
      self.Form().BaselineHDLCholesterolValue() != null &&
      (self.Form().BaselineHDLCholesterol() < 20 ||
        self.Form().BaselineHDLCholesterol() > 100)
    ) {
      i = self.Form().UnitOfMeasure()
        ? self.FormData.notifications.baselineHdlCholesterol[2]
        : self.FormData.notifications.baselineHdlCholesterol[1];
    } else {
      i = self.FormData.notifications.blank[0];
    }
    return i;
  ...
  self.baselineLdlValidation = ko.pureComputed(function () {
    var i = self.FormData.notifications.blank[0];
    if (
      self.Form().BaselineLDLCholesterolValue() == null &&
      self.baselineValidate() == true
    ) {
      ...
      self.Form().BaselineLDLCholesterolValue() != null &&
      !new RegExp("^([0-9]{1,3}|0)([,.]{1}[0-9]{1,3})?$").test(
        self.Form().BaselineLDLCholesterolValue()
      )
    ) {
      i = self.FormData.notifications.baselineLdlCholesterol[4];
    } else if (
      self.Form().BaselineLDLCholesterol() != null &&
      (self.Form().BaselineLDLCholesterol() < 30 ||
        self.Form().BaselineLDLCholesterol() > 300)
    ) {
      i = self.Form().UnitOfMeasure()
        ? self.FormData.notifications.baselineLdlCholesterol[2]
        : self.FormData.notifications.baselineLdlCholesterol[1];
    } else if (
      self.Form().BaselineLDLCholesterolValue() != null &&
      parseFloat(self.Form().BaselineLDLCholesterol().toFixed(3)) >
        parseFloat(
          (
            self.Form().BaselineTotalCholesterol() -
            self.Form().BaselineHDLCholesterol()
          ).toFixed(3)
        )
    ) {
      i = self.Form().UnitOfMeasure()
        ? self.FormData.notifications.baselineLdlCholesterol[3]
        : self.FormData.notifications.baselineLdlCholesterol[3];
    } else {
      i = self.FormData.notifications.blank[0];
    }
  ...
  self.baselineBloodPresureValidation = ko.pureComputed(function () {
    if (
      self.Form().BaselineBloodPressure() == null &&
      self.baselineValidate() == true
    ) {
      return self.FormData.notifications.baselineBloodPresure[0];
    } else if (
      self.Form().BaselineBloodPressure() != null &&
      !new RegExp("^[0-9]{1,3}$").test(self.Form().BaselineBloodPressure())
    ) {
      return self.FormData.notifications.baselineBloodPresure[2];
    } else if (
      self.Form().BaselineBloodPressure() != null &&
      (self.Form().BaselineBloodPressure() < 90 ||
        self.Form().BaselineBloodPressure() > 200)
    ) {
  ...
  self.recommendationType = ko.pureComputed(function () {
    var cvRisk =
      self.Form().TenYearCurrentRisk() != undefined
        ? parseFloat(self.Form().TenYearCurrentRisk())
        : undefined;
    ...
    for (var bpadviceProp in self.FormData.bpAdviceText) {
      ...
      if ("conditional" in self.FormData.bpAdviceText[bpadviceProp]) {
        var execute =
          "(function(){if(" +
          self.FormData.bpAdviceText[bpadviceProp].conditional +
          ")return self.FormData.bpAdviceText[bpadviceProp]; else return self.FormData.bpAdviceText.DUMMY;})()";
        advice.bpControl = eval(execute);
        if (advice.bpControl != self.FormData.bpAdviceText.DUMMY) break;
      }
    }
    for (var ldladviceProp in self.FormData.ldlAdviceText) {
      ...
      if ("conditional" in self.FormData.ldlAdviceText[ldladviceProp]) {
        var execute =
          "(function(){if(" +
          self.FormData.ldlAdviceText[ldladviceProp].conditional +
          ")return self.FormData.ldlAdviceText[ldladviceProp]; else return self.FormData.ldlAdviceText.DUMMY;})()";
        advice.ldlControl = eval(execute);
        if (advice.ldlControl != self.FormData.ldlAdviceText.DUMMY) break;
      }
    }
    for (var aspirinadviceProp in self.FormData.aspirinAdviceText) {
      ...
      if ("conditional" in self.FormData.aspirinAdviceText[aspirinadviceProp]) {
        var execute =
          "(function(){if(" +
          self.FormData.aspirinAdviceText[aspirinadviceProp].conditional +
          ")return self.FormData.aspirinAdviceText[aspirinadviceProp]; else return self.FormData.aspirinAdviceText.DUMMY;})()";
    ...
    for (var diabetesadviceProp in self.FormData.diabetesAdviceText) {
      ...
      if (
        "conditional" in self.FormData.diabetesAdviceText[diabetesadviceProp]
      ) {
        var execute =
          "(function(){if(" +
          self.FormData.diabetesAdviceText[diabetesadviceProp].conditional +
          ")return self.FormData.diabetesAdviceText[diabetesadviceProp]; else return self.FormData.diabetesAdviceText.DUMMY;})()";
    ...
    for (var smokingadviceProp in self.FormData.smokingAdviceText) {
      ...
      if ("conditional" in self.FormData.smokingAdviceText[smokingadviceProp]) {
        var execute =
          "(function(){if(" +
          self.FormData.smokingAdviceText[smokingadviceProp].conditional +
          ")return self.FormData.smokingAdviceText[smokingadviceProp]; else return self.FormData.smokingAdviceText.DUMMY;})()";
  ...
  self.Form().Smoker.subscribe(function (newValue) {
    ...
    if (self.Form().Smoker() == "Current") {
      ...
      var isSmoking = ko.utils.arrayFirst(
        self.Form().TratmentOne_selected(),
        function (item) {
          return item.therapyText === "Smoking Cessation";
        }
      );
      if (isSmoking) {
        self
          .Form()
          .TratmentOne_selected.remove(appmodel.FormData.treatmentOne[0]);
      }
      isSmoking = ko.utils.arrayFirst(
        self.Form().TratmentTwo_selected(),
        function (item) {
          return item.therapyText === "Smoking Cessation";
        }
      );
      if (isSmoking) {
        self
          .Form()
          .TratmentTwo_selected.remove(appmodel.FormData.treatmentTwo[0]);
      }
      isSmoking = ko.utils.arrayFirst(
        self.Form().TratmentThree_selected(),
        function (item) {
          return item.therapyText === "Smoking Cessation";
        }
      );
      if (isSmoking) {
        self
          .Form()
          .TratmentThree_selected.remove(appmodel.FormData.treatmentThree[0]);
      }
    }
  });
  ...
  self.gaRecordTreatments = function () {};
}
